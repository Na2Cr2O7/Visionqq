...
// 如果使用 .NET Core/.NET 5+，需添加 <UseWindowsForms>true</UseWindowsForms> 到 .csproj

namespace InputAutomation
{
    class Program
    {
        // === 常量 ===
        ...
        public static void DragFromToSimple(int x1, int y1, int x2, int y2)
        {
            ...
            MouseDown();
            MouseMove(x2, y2);
            Thread.Sleep(ScrollCount * 100); // scroll 变量用作延迟秒数？这里按 100ms * scroll
            MouseUp();
        }
    }
}